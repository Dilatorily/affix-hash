...
describe('cli', () => {
  ...
  it('shows the help menu if we don\'t pass any parameters', async () => {
    try {
      ...
      expect(fs.existsSync('tests/example-file.txt')).toBe(true);
      expect(fs.existsSync('tests/example-file.md')).toBe(true);
      expect(fs.existsSync('tests/.ignored')).toBe(true);
    }
  });

  it('affixes a hash to filenames', async () => {
    await execa('./src/cli.js', ['tests/**/*']);
    expect(fs.existsSync('tests/example-file.txt')).toBe(false);
    expect(fs.existsSync('tests/example-file.md')).toBe(false);
    expect(fs.existsSync('tests/.ignored')).toBe(true);
  });

  it('creates a manifest file of the renamed files', async () => {
    await execa('./src/cli.js', ['tests/**/*', '--manifest']);
    expect(fs.existsSync('manifest.json')).toBe(true);
    rimraf.sync('manifest.json');
  });

  it('uses the configured manifest filename', async () => {
    await execa('./src/cli.js', ['tests/**/*', '--manifest', '--manifest-name', 'test.json']);
  ...
  it('does not modify the files with the dry run parameter', async () => {
    await execa('./src/cli.js', ['\'tests/**/*\'', '--dry-run']);
    expect(fs.existsSync('tests/example-file.txt')).toBe(true);
    expect(fs.existsSync('tests/example-file.md')).toBe(true);
    expect(fs.existsSync('tests/.ignored')).toBe(true);
  });
